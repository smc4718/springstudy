...
3. 크론식 
  ...
  3) 방식
    (1) ? : 설정 없음 (요일에서 사용)
    (2) * : 매 번 (초 분 시 일 월에서 사용)
    (3) - : 범위
    (4) / : 주기 
   4) 예시
    (1) 특정 시간 (금요일 19시 0분 0초에 동작)
      0 0 19 * * FRI
    (2) 특정 범위 (새벽 1시 0분 0초부터 매 분마다 동작)
      0 0-30 1 * * ?
    (3) 반복 (10초마다 동작)
      0/10 * * * * ?
    (4) 반복 (10분마다 동작)
    ...
    (5) 반복 (7시, 19시마다 동작)
      0 0 7,19 * * ?
4.크론식 만들기
★ cronmaker.com : 해당 사이트에서 자동으로 크론식을 만들어 준다.
*/
...
public class ContactScheduler {
  ...
  @Scheduled(cron="0/5 * * * * ?")  // 0/5 : 0초부터 5초마다 찍기.
  public void doSomething() {
    
    // 한 시간마다 가장 예전에 등록된 연락처를 삭제하는 스케쥴러
